...
img_3 = cv2.imread('magic_circles/magic_circle_ccw.png', -1)
img_4 = cv2.imread('magic_circles/magic_circle_cw.png', -1)
    ...
    if result.multi_hand_landmarks:
        for hand in result.multi_hand_landmarks:
            ...
            if (ratio > 1.3):
                    ...
                    M3 = cv2.getRotationMatrix2D(cen, round(deg), 1.0)
                    M4 = cv2.getRotationMatrix2D(cen, round(360 - deg), 1.0)
                    rotated1 = cv2.warpAffine(img_1, M1, (wid, hei))
                    rotated2 = cv2.warpAffine(img_2, M2, (wid, hei))
                    rotated3 = cv2.warpAffine(img_3, M3, (wid, hei))
                    rotated4 = cv2.warpAffine(img_4, M4, (wid, hei))
                    if (diameter != 0):
                        img = transparent(rotated1, x1, y1, shield_size)
                        img = transparent(rotated2, x1, y1, shield_size)
                        img = transparent(rotated3, x1, y1, shield_size)
                        img = transparent(rotated4, x1, y1, shield_size)
